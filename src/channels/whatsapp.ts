...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // Raw metadata for Baileys sender-key logic — keeps @lid JIDs intact so Baileys
    // uses @lid encoding for LID-addressed groups (matching v1 Baileys 6.7.x behaviour).
    const rawGroupMetadataCache = new Map<string, { metadata: GroupMetadata; expiresAt: number }>();

    async function getRawGroupMetadata(jid: string): Promise<GroupMetadata | undefined> {
      if (!jid.endsWith('@g.us')) return undefined;
      const cached = rawGroupMetadataCache.get(jid);
      if (cached && cached.expiresAt > Date.now()) return cached.metadata;
      const metadata = await sock.groupMetadata(jid);
      rawGroupMetadataCache.set(jid, {
        metadata,
